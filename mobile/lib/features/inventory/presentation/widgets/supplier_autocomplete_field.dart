...
class SupplierAutocompleteField extends ConsumerStatefulWidget {
  final String initialValue;
  final ValueChanged<String> onSaved;
  final ValueChanged<VendorLedger>? onSupplierSelected;
  final String label;
  final bool hasError;
  /// When true, shows the full list of recent suppliers when field is focused
  ...
  @override
  ConsumerState<SupplierAutocompleteField> createState() =>
      _SupplierAutocompleteFieldState();
}

class _SupplierAutocompleteFieldState
    extends ConsumerState<SupplierAutocompleteField> {
  late TextEditingController _controller;
  final FocusNode _focusNode = FocusNode();
  Timer? _debounceTimer;
  bool _isNew = false;
  VendorLedger? _selectedParty;
  ...
  void _matchInitialParty(String value) {
    ...
    final ledgers = ref.read(vendorLedgerProvider).ledgers;
    final match = ledgers.where((l) =>
        l.vendorName.trim().toLowerCase() == value.trim().toLowerCase()).firstOrNull;
  ...
  String _formatLastVisit(VendorLedger party) {
  ...
  Widget build(BuildContext context) {
    final ledgers = ref.watch(vendorLedgerProvider).ledgers;

    // Sort by most recently updated
    final sortedLedgers = List<VendorLedger>.from(ledgers)
      ..sort((a, b) {
    ...
    return Autocomplete<VendorLedger>(
      displayStringForOption: (option) => option.vendorName,
      ...
      optionsBuilder: (TextEditingValue textEditingValue) {
        ...
        if (textEditingValue.text.isEmpty) {
          ...
          return const Iterable<VendorLedger>.empty();
        }
        return sortedLedgers.where((VendorLedger option) {
          return option.vendorName
              .toLowerCase()
              .contains(textEditingValue.text.toLowerCase());
        });
      },
      onSelected: (VendorLedger selection) {
        _controller.text = selection.vendorName;
        setState(() {
          _selectedParty = selection;
          _isNew = false;
        });
        if (widget.onSupplierSelected != null) {
          widget.onSupplierSelected!(selection);
        }
        _save();
      },
      fieldViewBuilder:
              ...
              child: ClipRRect(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    final VendorLedger party = options.elementAt(index);
                        ...
                        child: Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Center(
                                child: Text(
                                  party.vendorName.isNotEmpty
                                      ? party.vendorName[0].toUpperCase()
                                      : '?',
                                  style: TextStyle(
          ...
          children: [
            ...
            if (hasBalance) ...[
              ...
              Container(
                ...
                child: Text(
                  '${_formatBalance(balance)} payable',
                  style: TextStyle(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              const SizedBox(width: 5),
              Text(
                'New Supplier — will be added to Parties',
                style: TextStyle(
                  color: context.primaryColor,
                  fontSize: 11,
                  fontWeight: FontWeight.w700,
