...
class MapCustomerItemRequest(BaseModel):
    """Request model for mapping a stock item to a customer item name"""
    stock_level_id: Optional[int] = None
    part_number: str
    internal_item_name: Optional[str] = None
    customer_item_name: str
...
def get_unmapped_stock_items(
    search: Optional[str] = Query(None),
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    Get stock_levels items that have no customer_items mapping yet.
    Used by the mobile 'Map Items' screen to show items needing mapping.
    Mirrors the web's 'Items to Map' counter (items where customer_items is null/empty).
    """
    ...
    try:
        query = db.client.table("stock_levels")\
            .select("id, internal_item_name, part_number, current_stock, manual_adjustment, priority, customer_items")\
            .eq("username", username)\
            .or_("customer_items.is.null,customer_items.eq.")

        if search:
            query = query.ilike("internal_item_name", f"%{search}%")

        response = query.order("internal_item_name").execute()
        items = response.data or []

        # Build response in a shape the mobile can consume
        result = []
        for item in items:
            on_hand = (item.get("current_stock") or 0) + (item.get("manual_adjustment") or 0)
            result.append({
                "customer_item": item.get("internal_item_name") or "",
                "part_number": item.get("part_number") or "",
                "occurrence_count": max(1, int(on_hand)),
                "total_qty": float(on_hand),
                "variation_count": 1,
                "variations": [],
                "normalized_description": None,
                "stock_level_id": item.get("id"),
                "priority": item.get("priority") or "P2",
            })

        logger.info(f"Found {len(result)} unmapped stock items for {username}")
        ...
        logger.error(f"Error getting unmapped stock items: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/map-customer-item")
def map_stock_item_to_customer(
    request: MapCustomerItemRequest,
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    Save a customer_items mapping for a stock_levels item.
    Used by the mobile Map Items screen to confirm a mapping.
    Also creates a vendor_mapping_entries record so stock recalculation picks it up.
    """
    username = current_user.get("username")
    db = get_database_client()
    ...
    try:
        ...
        internal_item_name = request.internal_item_name
        customer_item_name = request.customer_item_name

        # 1. Update stock_levels.customer_items
        db.client.table("stock_levels")\
            .update({"customer_items": customer_item_name})\
            .eq("username", username)\
            .eq("part_number", part_number)\
            .execute()

        # 2. Upsert into vendor_mapping_entries so stock calc uses it
        db.client.table("vendor_mapping_entries")\
            .upsert({
                "username": username,
                "vendor_description": internal_item_name or customer_item_name,
                "part_number": part_number,
                "customer_item_name": customer_item_name,
                "status": "Added",
                "created_at": datetime.utcnow().isoformat(),
                "updated_at": datetime.utcnow().isoformat(),
            }, on_conflict="username,part_number")\
            .execute()

        logger.info(f"Mapped stock item '{part_number}' -> '{customer_item_name}' for {username}")
        ...
        logger.error(f"Error mapping stock item: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_stock_levels(
    ...
    try:
        ...
        logger.info(f"Retrieved {len(items)} stock levels for {username} (Total: {total})")
