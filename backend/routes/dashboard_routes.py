...
async def get_revenue_summary(
    ...
    try:
        # Load user config for column mappings — fall back to safe defaults if missing
        config = get_user_config(username) or {}
        revenue_config = config.get("dashboard_visuals", {}).get("revenue_metrics", {})

        # Get column names from config (sensible defaults so query always runs)
...
async def get_revenue_trends(
    ...
    try:
        # Load user config — fall back to safe defaults if missing
        config = get_user_config(username) or {}
        revenue_config = config.get("dashboard_visuals", {}).get("revenue_metrics", {})
...
async def get_stock_summary(
    ...
    try:
        # Load user config — fall back to safe defaults if missing
        config = get_user_config(username) or {}
        stock_config = config.get("dashboard_visuals", {}).get("stock_metrics", {})
...
async def get_dashboard_kpis(
    ...
    try:
        ...
        # --- Pending Actions KPI ---
        # Count unique receipts sitting in verification_dates (awaiting review/sync)
        try:
            pending_resp = db.client.table("verification_dates") \
                .select("receipt_number") \
                .eq("username", username) \
                .execute()
            pending_rows = pending_resp.data or []
            # Count unique receipt numbers
            unique_receipts = set(
                r.get("receipt_number") for r in pending_rows if r.get("receipt_number")
            )
            current_pending = len(unique_receipts)
        except Exception as pending_err:
            logger.warning(f"Could not fetch pending actions for {username}: {pending_err}")
            current_pending = 0
        prev_pending = 0
        pending_change = 0
...
async def get_daily_sales_volume(
    ...
    try:
        ...
        amount_col = revenue_config.get("amount_column", "amount")
        receipt_col = revenue_config.get("receipt_column", "receipt_number")
        type_col = revenue_config.get("type_column", "type")  # Get type column
        data_source = revenue_config.get("data_source", "verified_invoices")
