...
async def delete_purchase_order(
    ...
    try:
        ...
        # Due to ON DELETE CASCADE on purchase_order_items (if configured), 
        # deleting the PO might delete items automatically, but we can explicitly delete items first just in case.
        db.client.table("purchase_order_items")\
            .delete()\
            .eq("po_id", po_id)\
            .execute()
        ...
        # Try to clean up the PDF file if it exists locally
        pdf_path = po_response.data[0].get("pdf_file_path")
        if pdf_path and os.path.exists(pdf_path):
            try:
                os.remove(pdf_path)
            except OSError as e:
                logger.warning(f"Failed to delete PO PDF {pdf_path}: {e}")
                
        logger.info(f"Successfully deleted PO {po_id}")
...
def generate_po_pdf(po_record: Dict, items: List[Dict], username: str) -> BytesIO:
    ...
    # Company Header
    elements.append(Paragraph("<b>MATERIAL REQUEST</b>", title_style))
    elements.append(Spacer(1, 0.2*inch))
    
    # MR Details Table
    po_details_data = [
        ['MR Number:', po_record.get('po_number', '')],
    ...
    for idx, item in enumerate(items, 1):
        ...
        table_data.append([
            ...
            str(qty),
            f"Rs. {unit_price:,.2f}" if unit_price > 0 else 'Rs. --',
            f"Rs. {line_total:,.2f}" if unit_price > 0 else 'Rs. --'
        ])
